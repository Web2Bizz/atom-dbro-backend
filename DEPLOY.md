...
8. Экспортирует образ в `/tmp/atom-dbro-image.tar.gz` на сервере
9. Скачивает готовый образ обратно через SCP
10. Очищает временный файл образа на сервере сборки
11. Загружает артефакт для следующих этапов

#### Этап 2: Передача образа (Transfer)

1. Скачивает артефакт `image.tar.gz` из предыдущего этапа
2. Подключается к серверу деплоя по SSH
3. Передаёт образ на сервер деплоя через SCP в `/tmp/image.tar.gz`
4. Проверяет успешность передачи

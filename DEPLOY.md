...
4. Экспортирует образ в архив `image.tar.gz`
...
8. Экспортирует образ в `/tmp/atom-dbro-image.tar.gz` на сервере сборки
9. Скачивает образ для валидации (образ остаётся на сервере сборки)
10. **Образ остаётся на сервере сборки** для следующего этапа

**Локальная сборка (по умолчанию):**
1. GitHub Actions запускает workflow
2. Определяет режим сборки (локальный, если `BUILD_SERVER_*` секреты не настроены)
3. Собирает Docker образ на GitHub runner
4. Экспортирует образ в `image.tar.gz`
5. Загружает образ в GitHub Artifacts
...
- Запускается только если `use_remote != 'true'`
- Скачивает артефакт `image.tar.gz` из GitHub Artifacts
- Подключается к серверу деплоя по SSH
- Передаёт образ на сервер деплоя через SCP в `/tmp/image.tar.gz`
- Проверяет успешность передачи

**Transfer SCP Job** (для удалённой сборки):
- Запускается только если `use_remote == 'true'`
- Подключается к серверу сборки по SSH
- Скачивает образ `/tmp/atom-dbro-image.tar.gz` с сервера сборки
- Подключается к серверу деплоя по SSH
- Передаёт образ на сервер деплоя через SCP в `/tmp/image.tar.gz`
- Очищает образ с сервера сборки после успешной передачи
- Проверяет успешность передачи

**Важно:** Оба transfer job'а работают параллельно, но запускается только один из них в зависимости от режима сборки. Deploy job ждёт завершения обоих (один будет успешным, другой пропущен).
...
3. Загружает образ в Docker (`docker load -i /tmp/image.tar.gz`)
4. Удаляет временный файл образа
...
**Ошибка**: `scp` не может скачать `image.tar.gz`
...
┌─────────────────────────────────────────────────────────────┐
│                    BUILD JOB                                 │
│  Определяет режим: use_remote = true/false                  │
│                                                              │
│  ┌──────────────────┐              ┌──────────────────┐    │
│  │  Remote Build    │              │  Local Build     │    │
│  │  (use_remote=1)  │              │  (use_remote=0)  │    │
│  └──────────────────┘              └──────────────────┘    │
│          │                              │                   │
│          ↓                              ↓                   │
...
**Путь 1: Удалённая сборка** (`BUILD_SERVER_*` настроены)
- `BUILD` → сборка на Build Server
- `TRANSFER-SCP` → передача Build Server → Deploy Server
- `TRANSFER-GITHUB` → пропущен
- `DEPLOY` → деплой приложения

**Путь 2: Локальная сборка** (по умолчанию)
- `BUILD` → сборка на GitHub runner + upload в Artifacts
- `TRANSFER-GITHUB` → скачивание из Artifacts + передача на Deploy Server
- `TRANSFER-SCP` → пропущен
- `DEPLOY` → деплой приложения

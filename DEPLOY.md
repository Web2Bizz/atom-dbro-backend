...
#### 2. Установка Docker на сервере сборки
...
4. Синхронизирует исходный код на сервер через `rsync`
5. Запускает сборку Docker образа на удалённом сервере
6. Экспортирует образ в `/tmp/atom-dbro-image.tar.gz` на сервере
7. Скачивает готовый образ обратно через SCP
8. Очищает временные файлы на сервере сборки
9. Загружает артефакт для следующих этапов
...
**Ошибка**: `rsync` не может синхронизировать файлы

**Решение**:
1. Убедитесь, что `rsync` установлен на сервере сборки:
   ```bash
   # Ubuntu/Debian
